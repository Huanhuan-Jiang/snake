...
void drawBodyAt(SDL_Renderer* sdl_renderer, std::deque<std::pair<int, int>> obj,
                int pixel_size) {
  for (auto& element : obj) {
    ...
    SDL_FRect rect = {static_cast<float>(logicalX * pixel_size),
                      ...
                      static_cast<float>(pixel_size)};
    SDL_SetRenderDrawColor(sdl_renderer, 42, 76, 101, 255);  // Red
...
void Game::render() {
  ...
  drawFoodAt(game_renderer_.getRenderer(), snake_.getFood(), pixel_size_);
  drawBodyAt(game_renderer_.getRenderer(), snake_.getBody().deque(),
             pixel_size_);
